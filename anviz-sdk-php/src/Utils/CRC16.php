...
class CRC16
{
    ...
    public static function compute(array $data, int $length): int
    {
        $crc = 0xFFFF;
        for ($i = 0; $i < $length; $i++) {
            $temp = ($crc ^ $data[$i]) & 0xFFFF;
            $crc = (($crc >> 8) ^ self::$crcTable[$temp & 0x00FF]) & 0xFFFF;
        }
        return $crc;
    }

    public static function validate(array $data, int $length): bool
    {
        if ($length < 2) return false;
        $calculatedCrc = self::compute($data, $length - 2);
        $receivedCrc = $data[$length - 2] | ($data[$length - 1] << 8);
